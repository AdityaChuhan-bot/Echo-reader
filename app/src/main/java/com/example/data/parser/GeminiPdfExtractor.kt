...
object GeminiPdfExtractor {
    ...
    /**
     * Extracts text from PDF using high-performance local on-device offline extraction.
     */
    suspend fun extractTextFromPdf(file: File): String = withContext(Dispatchers.IO) {
        Log.d(TAG, "Attempting local 100% offline text extraction...")
        try {
            ...
            Log.e(TAG, "Local offline extraction failed: ${e.message}", e)
        }
